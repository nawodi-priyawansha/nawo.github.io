import { Project, Testimonial, Education } from '@/lib/types'
...
const getAllEducations = async (): Promise<Education[]> => {
  try {
    const educationsPath = path.join(process.cwd(), '/content/educations')
    const educationsName = await fs.readdir(educationsPath)

    const educations = await Promise.all(
      educationsName.map(async (projectName) => {
        const filePath = path.join(educationsPath, projectName)
        const projectDetails = await fs.readFile(filePath, 'utf8')
        return JSON.parse(projectDetails)
      }),
    )
    ...
    educations.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
...
export { getAllProjects, getAllTestimonials, getAllEducations }
